...
export default function App() {
  ...
  useEffect(() => {
    ...
    const requestNotificationPermission = async () => {
      // Force request permission on mount if we're in the app
      await reminderService.requestPermission();
      // If we got permission or already had it, sync habits
      if (user && habits.length > 0) {
        reminderService.sync(habits);
      }
    };
      ...
      try {
        ...
        if (mounted) {
          if (session?.user) {
            ...
            await fetchHabits();
            // Automatically prompt for permission when the user is logged in
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (mounted) {
        ...
        if (session?.user) {
          fetchHabits();
          // Prompt for permission on successful login
          await requestNotificationPermission();
  ...
  }, [fetchHabits, habits.length, user]);
