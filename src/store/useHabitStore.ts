      ...
      fetchHabits: async () => {
        try {
          ...
          const formattedHabits: Habit[] = habitsData.map((h: any) => {
            ...
            // Auto-complete if the duration has passed, even if check-ins are missing
            const startDate = startOfDay(parseISO(habit.startDate));
            const today = startOfDay(new Date());
            const currentDay = differenceInDays(today, startDate) + 1;

            if (currentDay > habit.duration && habit.status === 'active') {
              habit.status = 'completed';
              // Update database in the background to persist this state
              supabase.from('habits').update({ status: 'completed' }).eq('id', habit.id).then();
            }
      ...
      toggleCheckIn: async (habitId, day) => {
        ...
        set((state) => {
          ...
          const updatedHabits = state.habits.map((h) => {
            ...
            const startDate = startOfDay(parseISO(h.startDate));
            const today = startOfDay(new Date());
            const currentDay = differenceInDays(today, startDate) + 1;

            const isCompleted = newCheckIns.length === h.duration || currentDay > h.duration;
